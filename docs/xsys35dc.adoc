...
*xsys35dc* [_options_] _aldfile_... [_system39.ain_] [_debuginfo_]
...
`System39.ain` and `...SA.ALD.symbols` debug information file) either from a
specified directory or directly from files provided on the command line. The
decompiler generates source files that are compatible with
xref:xsys35c.adoc[*xsys35c(1)*] and the official System 3.x SDK.
...
== Output
If a debug information file is provided and the `--address` option is not
specified, `xsys35dc` will write out the original source files contained in the
